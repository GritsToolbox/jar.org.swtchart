...
public class Axis implements IAxis {
    ...
    /** the number of skipping items in x-axis labels */
    private int numSkipXaxisLabel = 1;
    ...
    public void setRange(Range range, boolean update) {
        ...
        if (isValidCategoryAxis()) { 		//x-axis
            min = (int) range.lower;		//min val in x-axis tick (in the plot area)
            max = (int) range.upper;		//man val in x-axis tick (in the plot area)
            if (min < 0) {
                min = 0;
            }
            ...
            int plotwidth = chart.getPlotArea().getSize().x;	//plot area size
            
            /*
             * Adjust the number of skipping tick in x-axis 
             * according to the width of plot area and the number of items in the x-axis (in the current plot area size)
             */
    		if(max - min> 1) {
    			int xAxisLength = (int) max - (int) min; 
    			
    			if(plotwidth > 0){
        			if(plotwidth < 400) {
        				xAxisLength = xAxisLength * 3;
        			}else if(plotwidth < 600) {
        				xAxisLength = xAxisLength * 2;
        			}    				
    			}
    				
    			setSkipItemForXaxis(xAxisLength);
    			
    		}else {
    			xAxis.setSkipRangeForXaxisLabel(1);	
    		}
    ...
    public void zoomIn(double coordinate) {
        ...
        if (isValidCategoryAxis()) {
            ...
            int xAxisLength = (int) upper - (int) lower;
            setSkipItemForXaxis(xAxisLength);
    ...
    /**
     * Sets the number of skipping items in x-axis labels
     * 
     * @param numSkipXaxisLabel
     *            the number of skipping items in x-axis labels
     */
    public void setSkipRangeForXaxisLabel(int numSkipXaxisLabel) {
        this.numSkipXaxisLabel = numSkipXaxisLabel;
    }

    /**
     * Gets the number of skipping items in x-axis labels
     * 
     * @return number of skipping items in x-axis labels
     */
    public int getSkipRangeForXaxisLabel() {
        return numSkipXaxisLabel;
    }
    
    
    /**
     * set skip items in X-axis
     */
    protected void setSkipItemForXaxis(int xAxisLength) {
    	IAxis xAxis = chart.getAxisSet().getXAxis(0);	
		if(xAxisLength > 1) {
			if(xAxisLength > 1000) {
				xAxis.setSkipRangeForXaxisLabel(150);
			}else if(xAxisLength > 750) {
					xAxis.setSkipRangeForXaxisLabel(80);
			}else if(xAxisLength > 500) {
				xAxis.setSkipRangeForXaxisLabel(50);
			}else if(xAxisLength > 300) {
				xAxis.setSkipRangeForXaxisLabel(30);
			}else if(xAxisLength > 100) {
				xAxis.setSkipRangeForXaxisLabel(10);
			}else if(xAxisLength > 50) {
				xAxis.setSkipRangeForXaxisLabel(5);
			}else {
				xAxis.setSkipRangeForXaxisLabel(1);			
			}			
		}else {
